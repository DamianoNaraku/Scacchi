...
public class Partita2 {
	...
	private void eseguiComando(String fullcommand) {
		...
		switch(comando) {
			...
			case "test": case "t":
				...
				int newpos64 = this.stato.sca.convertitore(from);
				int newpos88 = this.stato.sca.convertitoreinverso(from);
				System.out.println("coordinate change 11-88 to 0-64: " + from + " --> " + newpos64);
				System.out.println("coordinate change 0-64 to 11-88: " + from + " --> " + newpos88);
				System.out.println("coordinate change twice: " + this.stato.sca.convertitore(newpos88) + " --> " + this.stato.sca.convertitoreinverso(newpos64));
				*/
				break;
			case "mostra": case "m":
				if (comandoArray.length != 3) { invalidCommand("lunghezza comando:" + comandoArray.length); break; }
				...
				if (null == p) { System.out.println("casella vuota"); break; }
	...
	private void invalidCommand(String message) {
		invalidCommand();
		if (null != message) System.err.println("messaggio:" + message);
	}
}
